...
void user_init(void) {
  ...
  printf("Free RAM: %d\r\n", system_get_free_heap_size());
  xTaskCreate(mg_task1, (const signed char *) "mongoose1",
              MG_TASK_STACK_SIZE / 4, /* in 32-bit words */
              NULL, MGOS_TASK_PRIORITY, NULL);
  xTaskCreate(mg_task2, (const signed char *) "mongoose2",
              MG_TASK_STACK_SIZE / 4, /* in 32-bit words */
              NULL, MGOS_TASK_PRIORITY, NULL);
